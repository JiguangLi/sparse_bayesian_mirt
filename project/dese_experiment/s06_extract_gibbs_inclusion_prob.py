...
def read_thetas(model_output_dir, burn_in):
    with open(model_output_dir.joinpath("gibbs_grade10.pkl"), 'rb') as handle:
        model = pickle.load(handle)
        thetas.append(model.ss_thetas(burn_in))
    with open(model_output_dir.joinpath("gibbs_trig_grade10.pkl"), 'rb') as handle:
        model = pickle.load(handle)
        thetas.append(model.ss_thetas(burn_in))
...
if __name__ == "__main__":
    ...
    gibbs_model_output_dir = pathlib.Path.home().joinpath(pathlib.Path("IBP_IRT", "models", "dese", "2022", "gibbs"))
    burn_in = 500
    thetas = []
    ...
    output_dir = pathlib.Path.home().joinpath(pathlib.Path("IBP_IRT", "paper_markdown", "DESE"))
