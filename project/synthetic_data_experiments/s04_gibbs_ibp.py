...
def parse_args(verbose: bool = False) -> typing.Dict[str, typing.Any]:
    ...
    parser = argparse.ArgumentParser("Fit Gibbs Samplers on Small Data")
...
if __name__ == "__main__":
    ...
    data_input_dir = pathlib.Path.home().joinpath(pathlib.Path("IBP_IRT", "data", "ibp"))
    model_output_dir = pathlib.Path.home().joinpath(pathlib.Path("IBP_IRT", "models", "ibp"))
    data = pd.read_feather(pathlib.Path(data_input_dir).joinpath("ibp_logit.feather"))
    print(data.shape)
