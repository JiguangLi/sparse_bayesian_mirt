...
# parse arguments
arguments <- arg_parser("Test Bifactor on IBP dataset") %>% 
  add_argument(
    "--model_output_dir", 
...
# read ibp dataset
response_data <- arrow::read_feather(file.path(arguments[["data_output_dir"]], "ibp_logit.feather"))
data <- readRDS(file.path(arguments[["data_output_dir"]], "ibp_probit.rds"))
true_k <- 5
