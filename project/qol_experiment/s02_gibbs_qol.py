...
def parse_args(verbose: bool = False) -> typing.Dict[str, typing.Any]:
    ...
    parser.add_argument("--model_output_dir", default= pathlib.Path.home().joinpath(pathlib.Path("IBP_IRT", "models", "qol")))
...
if __name__ == "__main__":
    ...
    data_input_dir = pathlib.Path.home().joinpath(pathlib.Path("IBP_IRT", "data", "qol"))
    data_filename = "QOL.txt"
    with open(data_input_dir.joinpath(data_filename), 'r') as file:
        lines = file.readlines()
    cleaned_lines = [line.strip() for line in lines]
    data = pd.DataFrame(np.array([[int(num) for num in line] for line in cleaned_lines]),
                        columns=["item" + str(i) for i in range(1, 36)])
    binary_response = (data > 4).astype(int)
    print(binary_response.shape)
    large_k = 10
    burn_in = 500
    loading_constraints_large = bmirt.util.lower_trig_constraints(data.shape[1], large_k)
    # Gibbs sampler, large k, not triangularize the loading
    model1 = bmirt.MCMC_MIRT(binary_response, dim=large_k, num_samples=2000, alpha_prior="adaptive-ss",
                             loading_constraints={})
    model1.fit()
    save_results(model1, pathlib.Path(arguments["model_output_dir"]), "gibbs_large", burn_in)
    del model1
    # Gibbs sampler, large k, triangularize the loading
    model2 = bmirt.MCMC_MIRT(binary_response, dim=large_k, num_samples=2000, alpha_prior="adaptive-ss",
                            loading_constraints = loading_constraints_large)
    model2.fit()
    save_results(model2, pathlib.Path(arguments["model_output_dir"]), "gibbs_trig_large", burn_in)
    del model2
